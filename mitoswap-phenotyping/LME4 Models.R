...
#no Nuclear
FM1=lmer(df$WORMLENGTH ~ 1 + (1|batch) +(1|mito)+treatment+
           (1|nuclear:treatment)+(1|mito:treatment)+(1|nuclear:mito)+
           (1|nuclear:mito:treatment))
a = anova(FM,FM1)
chisq1 = a$Chisq[2]
...
#no Mito
FM2=lmer(df$WORMLENGTH ~ 1 + (1|batch) +(1|nuclear)+treatment+
           (1|nuclear:treatment)+(1|mito:treatment)+(1|nuclear:mito)+
           (1|nuclear:mito:treatment))
a = anova(FM,FM2)
chisq2 = a$Chisq[2]
...
#no Treatment
FM3=lmer(df$WORMLENGTH ~ 1 + (1|batch) +(1|nuclear)+(1|mito)+
           (1|nuclear:treatment)+(1|mito:treatment)+(1|nuclear:mito)+
           (1|nuclear:mito:treatment))
a = anova(FM,FM3)
chisq3 = a$Chisq[2]
...
FM4=lmer(df$WORMLENGTH ~ 1 + (1|batch) +(1|nuclear)+(1|mito)+treatment+
...
#no Mito-treatment
FM5=lmer(df$WORMLENGTH ~ 1 + (1|batch) +(1|nuclear)+(1|mito)+treatment+
...
#no Mitonuclear
FM6=lmer(df$WORMLENGTH ~ 1 + (1|batch) +(1|nuclear)+(1|mito)+treatment+
...
#no GXGXE
FM7=lmer(df$WORMLENGTH ~ 1 + (1|batch) +(1|nuclear)+(1|mito)+treatment+
           (1|nuclear:treatment)+(1|mito:treatment)+(1|nuclear:mito))
a=anova(FM,FM7)
chisq7 = a$Chisq[2]
...
for(condition in conditions){
  ...
  batch = factor(subcondition$Batch)
  
  FM=lmer(subcondition$WORMLENGTH ~ 1 + (1|batch) +(1|nuclear)+(1|mito)+(1|nuclear:mito))
  #no Nuclear
  FM1=lmer(subcondition$WORMLENGTH ~ 1 + (1|batch) +(1|mito)+(1|nuclear:mito))
  a = anova(FM,FM1)
  p1 = a$`Pr(>Chisq)`[2]
  d1 = a$Df[2]
  print(d1)
  #no Mito
  FM2=lmer(subcondition$WORMLENGTH ~ 1 + (1|batch)+(1|nuclear)+(1|nuclear:mito))
  b = anova(FM,FM2)
  p2 = b$`Pr(>Chisq)`[2]
  d2 = b$Df[2]
  print(d2)
  #no Mitonuclear
  FM3=lmer(subcondition$WORMLENGTH ~ 1 + (1|batch) +(1|nuclear)+(1|mito))
  c=anova(FM,FM3)
  p3 = c$`Pr(>Chisq)`[2]
  d3 = c$Df[2]
  print(d3)
  tempdf = data.frame(Condition = condition, Nuclear = p1, Mito = p2, Mitonuclear = p3)
  table2 = rbind(table2, tempdf)
}
